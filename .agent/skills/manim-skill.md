...
仅支持 **Ubuntu / Debian**（apt）。基础镜像已固定为 Ubuntu 24.04，APT 镜像源由 Dockerfile 统一配置，脚本不再处理换源。

### 包管理

使用 `uv` 安装 Manim（与 Dockerfile 一致）。`/opt/venv` 由 `uv venv` 创建，默认不含 pip，因此不要改用 `pip install`。
