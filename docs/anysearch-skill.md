...
该脚本会自动完成：下载 → 解压 → 运行时检测 → API Key 配置 → 功能验证。脚本内置共享 API Key，开箱即用。
...
脚本内置共享 API Key（存储在 [scripts/init-anysearch.sh](scripts/init-anysearch.sh) 的 `ANYSEARCH_API_KEY` 变量中），安装时自动写入 `.env` 文件。
